...
int main()
{
    ...
    while(t--)
    {
        ...
        cin>>N; //= read_int();
        cnt = 0;
        for(int i=0;i<=upper && i<N;i++)
        {
            for(int j=0;j<=upper && j<N;j++)
            {
                for(int k=0;k<=upper && k<N;k++)
                {
                  //  cout<<i<<" "<<" "<<j<<" "<<k<<endl;
                    exp = (expo(i,d)%N + (expo(j,d)%N + expo(k,d)%N)%N)%N;
                    //cout<<exp<<endl;
                    if( exp == m)
                    {
                        //i , j , k
                        ...
                        if(upper == 2*N -1)
                        {
                            ...
                            if(upper-k > N)
                                K += (upper-k+1)/N;
                            cnt = (cnt%MOD + ((I%MOD * J%MOD)%MOD * K%MOD)%MOD)%MOD ;
                        }
                        else if(upper >= 2*N)
                        {
                            if(upper-i > N)
