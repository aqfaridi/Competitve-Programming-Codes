...
bigint combine_data(bigint l,bigint r)
{
    return  (l * r);
}

bigint mod;
bigint combine_datamod(bigint l,bigint r)
{
...
bigint query(int node,int b,int e,int i,int j)
{
    if(b>=i && e<=j)
        return tree[node];
    int mid = (b+e)/2;
    ...
    return combine_datamod(query(2*node,b,mid,i,j),query(2*node+1,mid+1,e,i,j)); 
}
