 ...
 * the dramatic advisor of ACM has decided to transfigure some
 * tragedies into comedies. Obviously, this work is very hard because
 * the basic sense of the play must be kept intact, although all the
 * things change to their opposites. For example the numbers: if any
 * number appears in the tragedy, it must be converted to its reversed
 * form before being accepted into the comedy play.
 *
 * Reversed number is a number written in arabic numerals but the
 * order of digits is reversed. The first digit becomes last and vice
 * versa. For example, if the main hero had 1245 strawberries in the
 * tragedy, he has 5421 of them now. Note that all the leading zeros
 * are omitted. That means if the number ends with a zero, the zero is
 * lost by reversing (e.g. 1200 gives 21). Also note that the reversed
 ...
 * reversing). Thus we must assume that no zeros were lost by
