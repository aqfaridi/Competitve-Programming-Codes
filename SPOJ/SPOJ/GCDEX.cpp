...
void calc_g()
{
    g[1] = 0;
    //g(n) = g(n-1) + S(n)
    //S(n) = sigma over all div of n(d*phi(n/d))    
    for(int i=1;i<MAX;i++)//all possible div
    {
        for(int j=2;i*j<MAX;j++)
            g[i*j] += i*phi[j];//n=i*j  (i is div of n)
    }
    for(int i=2;i<MAX;i++)
        g[i] += g[i-1];
}
int main()
{
    ...
    //g(n) = g(n-1) + S(n)
    //S(n) = sigma over all div of n(d*phi(n/d))    
    for(i=1;i<MAX;i++)//all possible div
    {
        for(j=2;i*j<MAX;j++)
            g[i*j] += i*phi[j];//n=i*j  (i is div of n)
    }
