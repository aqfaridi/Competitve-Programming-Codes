...
**/
string str;
int minn = INT_MAX,operations;
int main()
{
    int l;
    //partitioning logic
    //trying each possibility at each pointer
    while(getline(cin,str))
    {
        ...
        for(int i=0;i<=l;i++)
        {
            ...
            {
               if(str[j] == 'G') // for conversion to red
                   operations++;
            }
            for(int j=i;j<l;j++)// i to l-1
            {
               if(str[j] == 'R') // for conversion to green            
                    operations++;
            } 
            if(operations < minn)
                minn = operations;
