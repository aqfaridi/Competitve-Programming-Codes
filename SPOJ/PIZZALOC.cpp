...
/*
struct points { int x, y;} loc[50];
struct tri { int x, y, ppl;} sol[110];
*/
int loc[50][2],sol[110][3];
int setb[110],sizes[110];
int v[110][110];
...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<m;j++)
        {
            if((sqr(loc[j][0] - sol[i][0]) + sqr(loc[j][1] - sol[i][1])) <= r*r)
                v[j][sizes[j]++] = i;
        }
    }
    int cnt,bits,maxx=0,size,vsize;
    int subsets = (1<<m);
    int vv[110],ind;
