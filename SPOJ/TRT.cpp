...
LL backtrack(int l,int r)
{
    ...
    dp[l][r] = 1;
    LL year = n-(r-l);//remaing wine
    return dp[l][r] = max(year*arr[l] + backtrack(l+1,r),year*arr[r] + backtrack(l,r-1));   
}
LL bottomup()
{
    for(int i=1;i<=n;i++)
        dp[i][i] = arr[i]*n;//all wine consumed (basecase)
    int p;
    for(int i=1,k=1;i<=n-1;i++,k++)//k is used to build year
    {
        p = 1;
        for(int j=i+1;j<=n;j++)
        {
            dp[p][j] = max(dp[p+1][j] + (n-k)*arr[p] , dp[p][j-1]+(n-k)*arr[j]);
            p++;
        }
    }
    return dp[1][n];
